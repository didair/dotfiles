...
# Waybar setup script
# Usage: ./waybar/setup.sh [--update]

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOTFILES_DIR="$(dirname "$SCRIPT_DIR:-$HOME/.config/waybar")"
UPDATE_MODE=false
...
create_link "$SCRIPT_DIR/config.jsonc" "$XDG_CONFIG_HOME/waybar/config.jsonc"
create_link "$SCRIPT_DIR/style.css" "$XDG_CONFIG_HOME/waybar/style.css"
